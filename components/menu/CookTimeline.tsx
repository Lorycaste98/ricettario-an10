    ...
    <section className="rounded-2xl border border-white/25 bg-white/30 backdrop-blur-sm p-4 sm:p-5 space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
        ...
        <div className="ml-auto flex items-center gap-1">
          <button
            type="button"
            onClick={() => canZoomOut && setPxPerMin(ZOOM_PRESETS[zoomIdx + 1])}
            disabled={!canZoomOut}
            title="Riduci zoom"
            className="flex h-7 w-7 items-center justify-center rounded-lg border border-white/40 bg-white/50 text-sky-700 hover:bg-white/80 disabled:opacity-30 transition-colors"
          >
            <ZoomOut size={13} />
          </button>
          <button
            type="button"
            onClick={() => canZoomIn && setPxPerMin(ZOOM_PRESETS[zoomIdx - 1])}
            disabled={!canZoomIn}
            title="Aumenta zoom"
            className="flex h-7 w-7 items-center justify-center rounded-lg border border-white/40 bg-white/50 text-sky-700 hover:bg-white/80 disabled:opacity-30 transition-colors"
          >
      ...
      {/* Canvas scrollabile */}
      <div className="overflow-x-auto overscroll-x-contain rounded-xl border border-white/30 bg-white/25 pb-2">
        <div className="relative" style={{ width: widthPx }}>
          ...
          {/* Linea del servizio */}
          <div className="pointer-events-none absolute bottom-0 top-0 z-10 w-0.5 bg-orange-500/80" style={{ left: servePx }}>
...
function TimelineLane({
  ...
  const segWidths = schedule.segments.map((s) => (s.mins > 0 ? s.mins * pxPerMin : SLIVER_PX));
  ...
  return (
    <div className="px-0.5">
      ...
      <div ref={constraintsRef} className="relative h-8" style={{ width: windowWidthPx }}>
        <motion.div
          drag="x"
          dragConstraints={constraintsRef}
          dragMomentum={false}
          dragElastic={0}
          style={{ x, left: startPx, width: barWidth, touchAction: "none" }}
          ...
          className={`absolute top-0 flex h-8 cursor-grab items-stretch overflow-hidden rounded-lg shadow-sm active:cursor-grabbing ${
          ...
          {schedule.segments.length > 0 ? (
            schedule.segments.map((seg, i) => (
              <button
                ...
                title={`${seg.text} — ${formatClock(seg.startsAt)}`}
                style={{ width: segWidths[i] }}
                className={`relative shrink-0 border-r border-white/50 last:border-r-0 ${SEGMENT_COLOR[seg.kind]} hover:brightness-110 transition-[filter]`}
              >
                {seg.mins * pxPerMin > 44 && (
